...
from utils.default import channelInfo, dim
...
class Word2Vector(object):
    ...
    def get_movie_vector(self, update=None):
        ...
        movieVector = self.spark.sql("select movie_id, min(cate_id) cate_id, collect_set(weightingVector) vectors \
                                    from tempTable group by movie_id")
        movieVector = movieVector.rdd.map(avg).toDF(["movie_id", "cate_id", "movieVector"])
