...
from movie_portrait.cal_merge_weight import get_topK_weight
from movie_portrait.cal_textrank import rank_map
from movie_portrait.cal_tfidf import get_cv_idf_model
from utils.default import mf_topK, dim, m_topK, k, cv_path, idf_path, um_spark, movie_recall_db, movie_portrait_db, \
                            movie_original_db
from utils.filter_data import get_filter_data
from utils.cut_words import MovieCutWords
from utils.save_tohive import RetToHive
from movie_recall.movie_filter_recall import MovieFilterRecall
import logging
...
class UpdateMovie(object):
    ...
    movie_portrait_db = movie_portrait_db
    movie_original_db = movie_original_db
    ...
    def merge_movie_data(self, full=False):
        ...
        self.spark.sql("use {}".format(self.movie_original_db))
        import time
        import datetime
        ...
        del update_df
        gc.collect()
        self.spark.sql("use {}".format(self.movie_portrait_db))
        if full:
            """
            insertInto 标记一下
            """
            try:
                ...
                RetToHive(self.spark, summary_df, "{}".format(movie_portrait_db), "movie_feature")
        else:
    ...
    def generate_movie_label(self, summary_df, full=False):
        ...
        words_df = summary_df.rdd.mapPartitions(segmentation)
        # from pyspark.shell import sqlContext
        words_df = words_df.toDF(["movie_id", "cate_id", "title", "words", "create_time"])
        # words_df = sqlContext.createDataFrame(words_df, ["movie_id", "cate_id", "title", "words", "create_time"])
        import time
        import datetime
        today = datetime.date.today()
        _yesterday = today - datetime.timedelta(days=1)
        ...
        if not full:
            ...
            try:
                ...
                RetToHive(self.spark, words_df, "{}".format(self.movie_portrait_db), "cut_words")
        ...
        try:
            ...
            RetToHive(self.spark, keywordsByTFIDF, "{}".format(self.movie_portrait_db), "tfidf")
        ...
        try:
            ...
            RetToHive(self.spark, textrank_keywords_df, "{}".format(self.movie_portrait_db), "textrank")
    ...
    def get_movie_profile(self, textrank, tfidf, full=False):
        ...
        if not full:
            ...
            try:
                ...
                RetToHive(self.spark, topic_weight, "{}".format(self.movie_portrait_db), "topic_weights")
    ...
    def compute_movie_similar(self, movieProfile=None, refit=False):
        ...
        # print(index.ntotal)
        D, I = index.search(movie_vector_array, self.topK)  # 寻找相似向量， I表示相似用户ID矩阵， D表示距离矩阵
        # print(D[:5])
        # print(I[-5:])
    ...
    def get_filter_latest_recall(self, movie_similar):
        mf_recall = MovieFilterRecall(self.spark, self.cate_id)
        recall_df = mf_recall.get_filter_version_recall(movie_similar)
        recall_df = mf_recall.get_filter_hot_score_year(recall_df)
        """
