...
from movie_portrait import MovieCutWords, rank_map, get_topK_weight, get_cv_idf_model
from utils import mf_topK, channel, cate_id, dim, m_topK, k, cv_path, idf_path, um_spark, \
    movie_recall_db
from movie_recall import Word2Vector, MovieFilterRecall
import logging
...
class UpdateMovie(object):
    ...
    def get_movie_profile(self, textrank, tfidf, full=False):
        ...
        topic_weight = topK_idf_textrank.join(topK_tfidf_textrank, on=['movie_id', 'cate_id', 'keyword']) \
            .withColumnRenamed('keyword', 'topic').withColumn('timestamp', F.lit(yesterday_time))

        # 3、将主题词权重增量更新
        if not full:
            # 查看 topic_weights 是否已经写入当前数据， 如果已写入则 跳过写入，否则正常写入当前数据
            df = self.spark.sql("select movie_id from topic_weights where timestamp = '{}'".format(yesterday_time))
            if df.rdd.count():
                pass
            else:
                """
    ...
    def compute_movie_similar(self, movieProfile=None, refit=False):
        ...
        self.spark.sql("use {}".format(movie_recall_db))
        ...
        # print(index.ntotal)
        D, I = index.search(update_movieVector_array, self.topK)  # 寻找相似向量， I表示相似用户ID矩阵， D表示距离矩阵
        # print(D[:5])
        # print(I[-5:])
        ...
        concat_df = pd.concat([update_movieVector_id, df, df2], axis=1)
    ...
    def get_filter_latest_recall(self, movie_similar):
        mf_recall = MovieFilterRecall(self.spark)
