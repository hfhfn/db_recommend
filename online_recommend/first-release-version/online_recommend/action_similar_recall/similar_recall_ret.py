from utils import cate_id, u_topK, m_topK, movie_recall_db, factor_db


class UserSimilarRecall(object):

    def __init__(self, portrait_db, recall_db, spark):
        self.spark = spark
        self.cate_id = cate_id
        self.u_topK = u_topK
        self.m_topK = m_topK
        self.portrait_db = portrait_db
    ...
    def get_user_similar_recall(self):
        action_weight = self.spark.sql(
            "select user_id, movie_id, cate_id, weight action_weight from {}.action_weight_normal"
                .format(self.portrait_db)).where('cate_id={}'.format(self.cate_id)).drop('cate_id')
