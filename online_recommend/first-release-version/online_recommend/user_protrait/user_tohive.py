from user_protrait import merge_user_data, get_pre_table, pre_user_data
from utils import RetToHive, user_pre_db, user_protrait_db, u_spark
from user_protrait import get_merge_action, get_action_topic_sort, get_action_weight, get_action_topic_weight, \
    get_user_profile, normal_action_weight


class SaveUserProfile(object):
    pre_db = user_pre_db
    protrait_db = user_protrait_db
    spark_app = u_spark
    ...
    def save_pre_userdata(self, table_type, table_num, start_num=0):
        ...
        while i < table_num:  # 修改 i 初始值和while循环的结束值来控制循环表的数量
            ...
            ret = pre_user_data(table_type, pre_table)
            # count = ret.count()
            # if count == 0:
    ...
    def save_action_weight(self):
        ...
        RetToHive(self.spark_app, action_weight_ret, self.protrait_db, action_weight_table)
    ...
    def save_action_weight_normal(self):
        ...
        :return:
        """
        action_weight_ret = normal_action_weight(self.protrait_db)
    ...
    def save_action_topic_weight(self):
        ...
        :return:
        """
        action_topic_ret = get_action_topic_weight(self.protrait_db)
        action_topic_table = 'action_topic_weight'
        RetToHive(self.spark_app, action_topic_ret, self.protrait_db, action_topic_table)
    ...
    def save_action_topic_sort(self):
        ...
        :return:
        """
        action_topic_sort_ret = get_action_topic_sort(self.protrait_db)
        action_topic_sort_table = 'action_topic_sort'
        RetToHive(self.spark_app, action_topic_sort_ret, self.protrait_db, action_topic_sort_table)
    ...
    def save_user_profile(self):
        ...
        """
        topK = 30
        user_profile_ret = get_user_profile(self.protrait_db, topK)
        user_profile_table = 'user_profile'
        RetToHive(self.spark_app, user_profile_ret, self.protrait_db, user_profile_table)
