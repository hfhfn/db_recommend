from utils import cate_id, u_topK, pre_topK, factor_db, movie_portrait_db


class UserRecall(object):

    def __init__(self, user_portrait_db, user_recall_db, spark):
        self.spark = spark
        ...
        self.movie_portrait_db = movie_portrait_db
            ...
            for row in partition:
                try:
                    ...
                    for key, value in movie_li:
                        yield row.user_id, row.cate_id, key, round(float(row.weight * value), 8)
                except:
                    # i += 1
                    # print('{}'.format(i) * 20)
                    # print(row.movie_li)
    ...
    def get_recall_ret(self):
        ...
        def map(row):
            ...
            return row.user_id, row.cate_id, row.movie_id, round(float(weight), 8)
    ...
    def get_user_recall_hot_score_year_factor(self):
        recall_df = self.spark.sql(
            "select * from {}.user_filter_version_recall_{}".format(self.user_recall_db, self.cate_id))
        hot_df = self.spark.sql(
            'select aid movie_id, title, cid, play_num, factor hot_factor from {}.movie_hot_factor'.format(
                self.factor_db)).where('cid={}'.format(self.cate_id)).drop('cid')
        score_df = self.spark.sql(
            'select aid movie_id, cid, score, factor score_factor from {}.movie_score_factor'.format(self.factor_db)) \
            .where('cid={}'.format(self.cate_id)).drop('cid')
        year_df = self.spark.sql('select * from {}.movie_year_factor'.format(self.factor_db))

        tmp_df = recall_df.join(hot_df, on='movie_id', how='left')
        tmp_df = tmp_df.join(score_df, on='movie_id', how='left')
        tmp_df = tmp_df.join(year_df, on='movie_id', how='left')

        def _map(row):
            weight = row.weight * row.hot_factor * row.score_factor * row.factor
            return row.user_id, row.cate_id, row.movie_id, row.title, row.year, row.play_num, row.score, \
                   round(float(weight), 8)

        recall_df = tmp_df.rdd.map(_map).toDF(
            ['user_id', 'cate_id', 'movie_id', 'title', 'year', 'play_num', 'score', 'weight'])
        import pyspark.sql.functions as fn
        # recall_df.agg(*[(1 - (fn.count(c) / fn.count('*'))).alias(c + '_missing') for c in recall_df.columns]).show()
        from pyspark.sql import Window
        recall_df = recall_df.withColumn("sort_num",
                                         fn.row_number().over(
                                             Window.partitionBy("user_id").orderBy(recall_df["weight"].desc())))
        return recall_df
