...
from utils import userDict_path, get_stopwords_list, movie_protrait_db
...
def rank_map(partition, topK):
    ...
    for row in partition:
        tags = textrank_model.textrank(row.summary, topK=topK, withWeight=True, allowPOS=allowPOS, withFlag=False)
        for tag in tags:
            # yield row.id, industry, tag[0], tag[1]
            yield row.id, row.cid, row.title, tag[0], tag[1]
...
def get_textrank():
    ...
    # 得到movie预处理数据结果
    ret = spark.sql("select * from {}.movie_feature".format(movie_protrait_db))
    ...
    movie_tag_weights = movie_tag_weights.toDF(["movie_id", "cate_id", "title", "tag", "textrank"])
...
def get_topK_textrank(topK):
    ...
    mapPartitions = partial(_mapPartitions, topK=topK)
    movie_tag_weights = ret.rdd.mapPartitions(mapPartitions)
