...
from movie_protrait.cal_merge_weight import idf_textrank
...
def get_keyword_weights():
    ...
    merge_keywords = spark.sql("select movie_id, min(cate_id) cate_id, collect_list(keywords) keywords, \
...
def get_profile():
    ...
    spark.sql("use {}".format(movie_protrait_db))
...
def normal_topic_weights():
    topic_weights = spark.sql("select * from movie_protrait.topic_weights")
