...
def _get_topK_tfidf():    # 弃用， 参考
    TOPK = 20
    tfidf_ret = spark.sql(
        "select movie_id, collect_list(keyword) keywords, collect_list(tfidf) tfidf from portrait.tfidf group by movie_id")
    # tfidf_ret = get_tfidf()
    ...
    # 分组排序， 参考
    # doc = spark.sql("select * from portrait.tfidf")
...
def get_topK_tfidf(topK):
    ...
    WHERE   T.RNUM <= {}""".format(movie_portrait_db, topK)
