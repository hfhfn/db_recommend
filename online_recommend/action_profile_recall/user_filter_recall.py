...
from utils.default import pre_topK, u_topK, movie_original_db, factor_db
# from pyspark.sql.types import *


class UserFilterRecall(object):

    def __init__(self, pre_db, recall_db, spark, cate_id):
        self.spark = spark
    ...
    def get_filter_version_recall(self):
        ...
        # 对title分词，有可能返回空列表
        recall_df = FilterCutWords(cate_id=self.cate_id, filter='user').get_words(recall_df).filter('title_len < 15')
        # 分词为空的直接留下，不做处理
