from utils.default import u_spark, user_recall_db, pre_topK, u_topK, user_portrait_db, user_pre_db
from .topic_movie_dict import get_inverted_table
from .user_recall_ret import UserRecall
from .user_filter_recall import UserFilterRecall
...
class SaveUserRecall(object):
    ...
    def __init__(self, cate_id):
        ...
        self.user_filter_recall = UserFilterRecall(self.pre_db, self.recall_db, self.spark_app, cate_id)
    ...
    def save_action_stat(self):
        ...
        RetToHive(self.spark_app, action_stat_ret, self.recall_db, action_stat_table)

    def save_action_stat_cate(self):
        # 分类保存用户行为数据的统计结果    1969 => 62966
        action_stat_ret = self.spark_app.sql("select * from {}.action_stat".format(self.recall_db)).where(
