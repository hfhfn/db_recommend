...
from utils import MovieDataApp, movie_portrait_db, movie_model_path
...
def get_cv_idf_model(doc, cut_field):
    # doc = get_words()
    # doc = spark.sql('select * from {}.cut_words'.format(movie_portrait_db))

    # count值计算
    # 这里将所有出现过的词都统计出来，这里最多会有n * 2个词(音乐有100多w条，暂定平均有2个不重复的词)
    cv = CountVectorizer(inputCol=cut_field, outputCol="rawFeatures", vocabSize=1000000 * 2, minDF=1.0)
    cv_model = cv.fit(doc)
    # 保存，加载模型
    cv_model.write().overwrite().save("{}{}_CV.model".format(movie_model_path, cut_field))
    ...
    # 保存，加载模型
    idfModel.write().overwrite().save("{}{}_IDF.model".format(movie_model_path, cut_field))


def get_tfidf(cut_field, refit=False):
    """
    计算tfidf
    :return: 【dataframe】
    """
    doc = spark.sql('select * from {}.cut_{}'.format(movie_portrait_db, cut_field))

    if refit:
        get_cv_idf_model(doc, cut_field)
    from pyspark.ml.feature import IDFModel
    idfModel = IDFModel.load("{}{}_IDF.model".format(movie_model_path, cut_field))
    from pyspark.ml.feature import CountVectorizerModel
    cv_model = CountVectorizerModel.load("{}{}_CV.model".format(movie_model_path, cut_field))
    ...
    def _tfidf(partition, kw_list):
        ...
        for row in partition:
            words_length = len(row.content)  # 统计文档中单词总数
            ...
            for index in indices_li:
                ...
                tfidf = values_li[index] / words_length
                yield row.song_id, row.song, row.singer, row.album, word, index, float(idf), round(float(tfidf), 4)
    ...
    keyword_tfidf = keyword_tfidf.toDF(["song_id", "song", "singer", "album", "keyword", "index", "idf", "tfidf"])
    # keyword_tfidf.show()
...
def get_topK_tfidf(cut_field, topK):
    """
    返回 topK个 tfidf结果
    :return: 【dataframe】
    """

    sql ="""SELECT movie_id, cate_id, title, keyword, tfidf
    FROM    ( SELECT    ROW_NUMBER() OVER ( PARTITION  BY t1.movie_id ORDER BY t1.tfidf DESC) AS RNUM ,
                        *
              FROM      {}.{}_tfidf t1
            ) AS T
    WHERE   T.RNUM <= {}""".format(movie_portrait_db, cut_field, topK)
