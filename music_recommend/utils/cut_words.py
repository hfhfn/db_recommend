...
class BaseCutWords(object):
    ...
    def __init__(self, cate_id=1969, filter=True, stopwords=True, add_source=False):
        """
        :param cate_id:
...
class MovieCutWords(BaseCutWords):
    ...
    def segmentation(self, partitions):
        for row in partitions:
            yield row.song_id, row.song, row.singer, row.album, self.cut_sentence(row.content)
    ...
    def get_words(self, sentence_df=None):
        ...
        """
        doc = sentence_df.rdd.mapPartitions(self.segmentation)
        doc = doc.toDF(["song_id", "song", "singer", "album", "content"])
        # print(doc.show())
        return doc
