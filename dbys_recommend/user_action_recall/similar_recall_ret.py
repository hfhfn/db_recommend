from utils import u_topK, m_topK, movie_recall_db, factor_db, movie_original_db, u_spark
...
class UserSimilarRecall(object):
    ...
    def get_user_similar_recall(self):
        action_weight = self.spark.sql(
            "select user_id, movie_id, cate_id, weight action_weight from {}.action_weight"
        ...
        def map(row):
            try:
                ...
                return row.user_id, row.movie_id, row.title, row.year, row.movie_id2, row.title2, row.year2, row.score, \
                       row.play_num, round(float(weight), 4)  # 如果使用归一化的权重需要保留8位小数
            except:
    ...
    def get_filter_same_recall(self):
        ...
        def map(row):
            ...
            return row.user_id, row.movie_id, row.title, row.year, row.movie_id2, row.title2, row.year2, row.score, \
                   row.play_num, round(float(weight), 4)
    ...
    def get_user_similar_latest_recall(self):
        ...
        ret = user_recall.join(movie_source, on='movie_id', how='left')
