...
def get_cv_idf_model(doc, cut_field):
    ...
    # count值计算
    # 这里将所有出现过的词都统计出来，这里最多会有n * 2个词(影视有40多w条，暂定平均有2个不重复的词)
    cv = CountVectorizer(inputCol="words", outputCol="rawFeatures", vocabSize=400000 * 2, minDF=1.0)
    cv_model = cv.fit(doc)
    # 保存，加载模型
...
def get_tfidf(cut_field, refit=False):
    ...
    :return: 【dataframe】
    """
    if cut_field == 'all_field':
        doc = spark.sql('select * from {}.movie_keywords'.format(movie_portrait_db))
    else:
