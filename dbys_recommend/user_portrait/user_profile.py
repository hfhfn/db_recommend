from utils import movie_portrait_db, UserDataApp, factor_db, movie_original_db
...
def get_action_weight(user_pre_db):
    ...
    # print(action_weights.where('movie_id is null').count())
    movie = spark.sql("select id movie_id, title, year from {}.db_asset".format(movie_original_db))
    action_weights = action_weights.join(movie, on='movie_id', how='left')
...
def get_action_topic_sort(user_portrait_db):
    ...
    def map(row):
        ...
        return row.user_id, row.cate_id, row.topic, round(float(weight), 4)  # 如果使用归一后的权重则需要保留8位小数
