from utils import dim, user_portrait_db, movie_model_path, movie_portrait_db
...
class Word2Vector(object):
    ...
    def __init__(self, spark, refit, channel_id=1969):

        self.refit = refit
        self.channel_id = channel_id
        self.user_portrait_db = user_portrait_db
        self.dim = dim
        self.word2vec_model_path = "{}word2vec/channel_{}.word2vec".format(movie_model_path, self.channel_id)
        self.spark = spark
        self.spark.sql("use {}".format(movie_portrait_db))
        self.cut_words_table = 'movie_keywords'

    def cal_word2vec(self):
        # 训练词向量模型

        words_df = self.spark.sql("select * from {} where cate_id={}".format(self.cut_words_table, self.channel_id))
        # words_df.show()
    ...
    def get_movie_vector(self, update=None):
        ...
        # 全量的电影画像权重，全量转vector
            ...
            topic_weight = self.spark.sql("select * from topk_all_field_tfidf where cate_id={}".format(self.channel_id))

        vectors = self.load_word2vec()
        # 合并计算（对每个词的向量进行权重加权）得到电影每个词的权重向量
        _movie_profile = topic_weight.join(vectors, vectors.word == topic_weight.keyword, "inner")
        # rdd.repartition(100) 可以改变存储分区数量
        movieKeywordVectors = _movie_profile.rdd.map(lambda row: (row.movie_id, row.cate_id, row.keyword,
                                                                  row.tfidf * row.vector)).toDF(
