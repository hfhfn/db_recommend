from utils import factor_db, FilterCutWords, RetToHive, user_recall_db, movie_original_db, movie_recall_db, \
        pre_topK, m_topK, online_db, als_db, movie_original_tb
    ...
    def get_filter_version_recall(self, update=None, filter='user'):
        ...
        if update:
            ...
            # action_recall
                ...
                recall_df = self.spark.sql("select * from {}.movie_similar_{}"
                                           .format(self.movie_recall_db, self.cate_id))

        # 增加一列数据， 电影title， 用来对同一个电影多版本和花絮进行过滤
        movie_df = self.spark.sql("select id, cid, title from {}.{}".format(movie_original_db, movie_original_tb))\
