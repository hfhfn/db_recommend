from utils import userDict_path, get_stopwords_list, tmpDict_path, originDict_path
...
class BaseCutWords(object):
    ...
    def __init__(self, cate_id=1969, filter=True, stopwords=True, cut_field='desc', add_source=False):
        """
        :param cate_id:
        :param filter:  True/False, | 'user'/'movie'
        :param stopwords:
        :param cut_field:  desc/title  |  分词不同的字段
        :param add_source:  分词结果为空的，是否把原文本添加为分词
        """
        self.filter = filter
...
class MovieCutWords(BaseCutWords):
    ...
    def segmentation(self, partitions):
        if self.cut_field == 'desc':
            for row in partitions:
                yield row.id, row.cid, row.title, self.cut_sentence(row.desc), row.create_time
        elif self.cut_field == 'title':
            for row in partitions:
                yield row.id, row.cid, row.title, self.cut_sentence(row.title), row.create_time
...
class FilterCutWords(BaseCutWords):
    ...
    def segmentation(self, partitions):
        ...
        for row in partitions:
            ...
            if self.filter == 'user':
                try:
                    yield row.user_id, row.movie_id, row.cos_sim, title, \
                            title_len, words, words_len
                except:
                    yield row.user_id, row.movie_id, row.weight, title, \
                            title_len, words, words_len
            else:
    ...
    def get_words(self, sentence_df=None):
        ...
        doc = sentence_df.rdd.mapPartitions(self.segmentation)
        if self.filter == 'user':
            doc = doc.toDF(["user_id", "movie_id", "cos_sim", "title", "title_len", "words", "words_len"]).dropna()
        else:
