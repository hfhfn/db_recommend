...
class SparkSessionBase(object):
    ...
    def _create_spark_session(self):
        ...
        config = (
            ...
            ("spark.executor.memoryOverhead", "28g"),
            ...
            ("spark.network.timeout", "300"),  # 设置worker接收executer或者task心跳反馈时间为5分钟

            # 文件缺失时忽略异常的参数
            # 处理spark DataSource表时
            ...
            # spark sql 调优
            ("spark.hadoopRDD.ignoreEmptySplits", "true"),  # 忽略空的splits，减小task的数量
            # 用于控制每个mapTask的输入文件，防止小文件过多时候，产生太多的task
            ("spark.hadoop.mapreduce.input.fileinputformat.split.minsize", "67108864"),
            # 读取数据时装入单个patition的最大字节数，默认128M
            # ("spark.sql.files.maxPartitionBytes", "268435456"),
            # 优化spark.sql处理小文件等处理机制
            # ("spark.sql.execution.arrow.enabled", "true"),   # 序列号友好和向量化，优化内存使用和计算效率
            # ("spark.sql.adaptive.join.enabled", "true"),  # 开启自适应join大小的调整，默认就时true
            # ("spark.sql.autoBroadcastJoinThreshold", "-1"),   # 设定广播表大小的max值为1G, -1为关闭广播
            ("spark.sql.autoBroadcastJoinThreshold", "64000000"),  # 设置join广播的阈值
            ("spark.sql.broadcastTimeout", "36000"),
            ("spark.sql.adaptive.enabled", "true"),  # 开启自适应调整shuffle大小
            # ("spark.sql.adaptive.shuffle.targetPostShuffleInputSize", "128000000"),  # 调整shuffle大小时的参照值，默认64M
            ("spark.sql.adaptive.skewedJoin.enabled", "true"),  # 处理数据倾斜
            ("spark.sql.hive.mergeFiles", "true"),  # 开启insert overwrite时 hive的merge功能，合并小文件
            ("spark.sql.adaptive.allowAdditionalShuffle", "true"),  # 设置允许处理时自动增加shuffle
        )
...
class FilterApp(SparkSessionBase):
    """
    过滤 版本，历史，因子加权
    """
    SPARK_APP_NAME = "filter"
    ENABLE_HIVE_SUPPORT = True
