...
def get_movie_hot_sort(delta_day=30):
    if delta_day == 0:   # 载入全部统计数据，计算movie_hot
        stat_play = spark.sql("select aid, date, count from {}.ty_asset_play".format(movie_original_db))
    else:
        import time
        from datetime import timedelta, datetime
        interval = timedelta(days=delta_day)
        timestamp = int(time.time()) - int(interval.total_seconds())
